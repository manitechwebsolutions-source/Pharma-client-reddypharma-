...
function AdminPage() {
  ...
  const login = async (e: React.FormEvent) => {
    ...
    if (!isSupabaseConfigured()) {
      toast.error("Connect Supabase first by setting VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY.");
      return;
    }
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Upload pequired products are here.
          </p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" className="rounded-full" onClick={() => { if (confirm("Reset to defaults?")) { reset(); toast.success("Reset to defaults"); } }}>
            <RotateCcw className="h-4 w-4 mr-2" /> Reset
          </Button>
          <Button variant="outline" className="rounded-full" onClick={onLogout}>
            <LogOut className="h-4 w-4 mr-2" /> Logout
          </Button>
          <Button className="rounded-full bg-gradient-to-r from-primary to-primary-glow" onClick={openNew}>
        ...
        {products.map((p) => (
          <div key={p.id} className="rounded-2xl border border-border/60 bg-card p-4 shadow-card">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <Badge className={`rounded-full ${p.available ? "bg-primary/10 text-primary" : "bg-destructive/10 text-destructive"}`}>
            ...
            <div className="mt-4 flex gap-2">
              <Button size="sm" variant="outline" className="flex-1 rounded-full" onClick={() => openEdit(p)}>
                <Pencil className="h-3.5 w-3.5 mr-1.5" /> Edit
              </Button>
              <Button size="sm" variant="outline" className="rounded-full text-destructive hover:text-destructive" onClick={() => del(p)}>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>{editing && products.find((x) => x.id === editing.id) ? "Edit product" : "New product"}</DialogTitle>
          </DialogHeader>
          {editing && (
            <div className="space-y-4">
              <div className="grid grid-cols-[1fr_auto] gap-3">
                ...
                <div>
                  <Label>Emoji</Label>
                  <Input value={editing.emoji} onChange={(e) => setEditing({ ...editing, emoji: e.target.value })} className="mt-1 w-20 text-center text-xl" />
                </div>
              </div>
              <div>
                <Label>Image</Label>
                <div className="mt-1 flex items-center gap-3">
                  {editing.image ? (
                    <img src={editing.image} alt="" className="h-20 w-20 rounded-xl object-cover border border-border/60" />
                  ) : (
          ...
          <DialogFooter>
            ...
            <Button className="rounded-full bg-gradient-to-r from-primary to-primary-glow" onClick={save}>
