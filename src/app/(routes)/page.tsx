...
function Hero({ heroBg }: { heroBg: string }) {
  return (
    <section className="relative overflow-hidden min-h-[640px] md:min-h-[720px] flex items-center">
      ...
      <div className="mx-auto max-w-7xl px-4 pt-24 pb-20 md:pt-32 md:pb-28 w-full relative z-20">
        <div className="max-w-2xl">
          ...
          >
            Genuine medicines, prescription assistance and doorstep delivery — care your family can
            count on.
          </motion.p>
          <motion.div
            initial={{ opacity: 0, y: 20 }}
          ...
          >
            <Button
              asChild
              size="lg"
              className="rounded-full bg-white text-primary hover:bg-white/90 text-base h-12 px-6"
            >
              <Link href="/request">
                Request Medicine <ArrowRight className="ml-1 h-4 w-4" />
              </Link>
            </Button>
            <Button
              asChild
              size="lg"
              variant="outline"
              className="rounded-full h-12 px-6 text-base bg-white/10 text-white border-white/30 hover:bg-white/20 hover:text-white"
            >
              <a
...
const WHY = [
  {
    ...
    desc: "Sourced from licensed distributors — every batch verified.",
  },
  { icon: Clock, title: "Fast Service", desc: "Skip the queue. Quick fulfilment on every order." },
  {
    icon: Truck,
    title: "Home Delivery",
    desc: "Doorstep delivery within 2km radius, often within an hour.",
  },
  {
    icon: HeartHandshake,
    title: "Trusted Locally",
    desc: "Family-run since years — known for honest, caring service.",
  },
];

const CATS = [
  { img: "/assets/medicines.png", name: "Medicines" },
  { img: "/assets/supplements.png", name: "Supplements" },
  { img: "/assets/personalcare.png", name: "Personal Care" },
  { img: "/assets/baby care.png", name: "Baby Care" },
  { img: "/assets/medicalwquipment.png", name: "Medical Equipment" },
];
...
const SERVICES = [
  { title: "Home Delivery", desc: "Free doorstep delivery  within 2km radius.", icon: Package },
  {
    ...
    desc: "Send a photo — we'll read it, dispense, and confirm.",
    icon: FileText,
  },
  {
    title: "Medicine Availability",
    desc: "Can't find a medicine? We'll source it for you fast.",
    icon: Search,
  },
  {
    title: "Health Products",
    desc: "Supplements, devices, baby care and daily essentials.",
    icon: HeartPulse,
  },
];
...
const REVIEWS = [
  {
    name: "Manohar Reddy.",
    text: "Some of the rare medicines are available and also available home delivery....",
    stars: 5,
  },
  { name: "Mohanrao Boligarla.", text: "Nice....", stars: 5 },
  {
    name: "Sneha M.",
    text: "Best pharmacy in the area. WhatsApp ordering is super convenient.",
    stars: 5,
  },
...
export default function Home() {
  return (
    <SiteLayout>
      <Hero heroBg="/assets/reddypharma.jpg" />
      ...
      <Section>
        <Reveal className="text-center max-w-2xl mx-auto">
          ...
          <p className="mt-3 text-muted-foreground">
            Built on transparency, speed, and personal service.
          </p>
        </Reveal>
        ...
        <div className="mt-12 grid md:grid-cols-2 lg:grid-cols-4 gap-5">
          {SERVICES.map((s, i) => (
            <Reveal key={s.title} delay={i * 0.08}>
              <motion.div
                whileHover={{ y: -6 }}
                className="rounded-2xl p-6 bg-card border border-border/60 shadow-card hover:shadow-glow transition-shadow h-full"
              >
                <div className="grid h-12 w-12 place-items-center rounded-xl bg-gradient-to-br from-primary to-primary-glow text-primary-foreground shadow-soft">
        ...
        <div className="mt-12 grid md:grid-cols-2 lg:grid-cols-4 gap-5">
          {REVIEWS.map((r, i) => (
            <Reveal key={r.name} delay={i * 0.08}>
              <motion.div
                whileHover={{ y: -4 }}
                className="glass rounded-2xl p-6 h-full shadow-card"
              >
        ...
        <Reveal>
          <div className="grid lg:grid-cols-2 gap-8 items-center">
            <div>
              ...
              <p className="mt-3 text-muted-foreground">
                Walk in, pick up, or ask our pharmacists anything. We're here to help.
              </p>
      ...
      <Section>
        <Reveal>
          <div
            className="relative overflow-hidden rounded-3xl p-10 md:p-16 text-center text-primary-foreground shadow-glow"
            style={{ background: "var(--gradient-primary)" }}
          >
            <div className="absolute -top-20 -right-20 h-64 w-64 rounded-full bg-white/10 blur-3xl" />
            <div className="absolute -bottom-20 -left-20 h-64 w-64 rounded-full bg-white/10 blur-3xl" />
            <h2 className="relative text-3xl md:text-5xl font-extrabold">
              Need medicines quickly?
            </h2>
            <p className="relative mt-3 opacity-90 max-w-xl mx-auto">
              Message us on WhatsApp or call now — we'll have it ready or at your doorstep.
            </p>
            <div className="relative mt-7 flex flex-wrap justify-center gap-3">
              <Button
                asChild
                size="lg"
                className="rounded-full bg-white text-primary hover:bg-white/90 h-12 px-6"
              >
                <a
                  href={waLink("Hi! I need medicines quickly.")}
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  WhatsApp Order
              ...
              <Button
                ...
                size="lg"
                variant="outline"
                className="rounded-full h-12 px-6 bg-transparent border-white/40 text-white hover:bg-white/10 hover:text-white"
              >
